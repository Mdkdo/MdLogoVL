document.addEventListener('DOMContentLoaded', () => {
    ...
    function resizeCanvas() {
        ...
        if (window.turtle) {
            window.turtle.originX = canvas.width / 2;
            window.turtle.originY = canvas.height / 2;
            window.turtle.draw();
        }
    }
    ...
    function updateStatus() {
        ...
        const pColor = window.turtle.color;
        ...
        const fColor = window.turtle.fillColor;
    ...
    turtleImgSelect.addEventListener('change', (e) => {
        ...
        if (val === 'default') window.turtle.setTurtleImage(null);
        else window.turtle.setTurtleImage(val);
        saveSettings();
    });
    ...
    function loadSettings() {
        ...
        if (saved) {
            ...
            if (turtleImgSelect.value === 'default') window.turtle.setTurtleImage(null);
            else window.turtle.setTurtleImage(turtleImgSelect.value);
        }
    }
    loadSettings();
    ...
    stopBtn.addEventListener('click', () => { window.turtle.stop(); });
    ...
    function runInline() {
        ...
        if (cmd.trim()) { window.executeSnippet(cmd); inlineCmdInput.value = ''; }
    }
    runInlineBtn.addEventListener('click', runInline);
    inlineCmdInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') runInline(); });
    ...
    codeEditor.addEventListener('keydown', (e) => {
        if (e.key === ' ') {
            ...
            if (lastWordMatch) {
                ...
                const keywords = ["donne", "declare", "si", "sinon", "tantque", "repete", "choisis", "autres", "classe", "pour", "fin", "case", "default", "stop", "continue", "rends", "répète"];
                const commands = ["av", "re", "td", "tg", "lc", "bc", "ve", "ct", "mt", "fcc", "fcb", "fcap", "fpos", "fca", "ftc", "écris", "ecris", "remplis", "tampon", "dégradé", "degrade", "opacité", "opacite", "fluide", "joue", "afficheImage", "showimage", "afficheVideo", "showvideo", "cercle", "polygone", "étoile", "etoile", "rvb", "fd", "bk", "rt", "lt", "pu", "pd", "cs", "clean", "home", "ht", "st", "ps", "setwidth", "circle", "rectangle", "ellipse", "polygon", "star", "stamp", "drawimage", "opacity", "smooth", "setxy", "setheading", "pencolor", "pc", "fillcolor", "fill", "canvascolor", "sin", "cos", "tan", "sqrt", "abs", "random"];

                const userProcs = [];
                const procSearch = text.replace(/("(?:[^"\\\n]|\\.)*"|'(?:[^'\\\n]|\\.)*'|`(?:[^\\`]|\\.)*`|\/\/.*|\/\*[\s\S]*?\*\/)/g, "");
                const procRegex = /\bpour\s+([a-zA-Z0-9_$À-ÿ]+)/gi;
                let m;
                while ((m = procRegex.exec(procSearch)) !== null) { userProcs.push(m[1].toLowerCase()); }
    ...
    openFileBtn.addEventListener('click', () => {
        ...
        input.onchange = (e) => {
            ...
            reader.onload = (ev) => { codeEditor.value = ev.target.result; window.updateHighlight(); };
    ...
    saveFileBtn.addEventListener('click', () => {
        let filename = prompt('Nom du fichier :', 'code.logo');
        if (!filename) return;
    ...
    function modifySelection(fn) {
        const start = codeEditor.selectionStart; const end = codeEditor.selectionEnd;
        const text = codeEditor.value; const before = text.substring(0, start);
        const selection = text.substring(start, end); const after = text.substring(end);
        const lines = selection.split('\n'); const newSelection = lines.map(fn).join('\n');
        saveState(); codeEditor.value = before + newSelection + after;
        codeEditor.selectionStart = start; codeEditor.selectionEnd = start + newSelection.length; window.updateHighlight();
    }
    ...
    document.querySelectorAll('.example-btn').forEach(btn => {
        btn.addEventListener('click', () => { codeEditor.value = btn.getAttribute('data-code'); window.updateHighlight(); });
    });
